...
CMD:service(playerid, params[])
{
	...
	{		
		if(pData[playerid][pMechVeh] == INVALID_VEHICLE_ID)
		{
			...
			if(pData[playerid][pActivityTime] > 5) return Error(playerid, "Anda sedang meservice kendaraan!");
			...
			foreach(new i : PVehicles)
			{
				if(pvData[i][cVeh] == vehicleid)
				{
					...
					Vehicle(GetVehicleOwner(i),"Kendaraan {7fffd4}%s{ffffff} process checked by %s", GetVehicleModelName(pvData[i][cModel]), ReturnName(playerid));
				}
			}
...
function TireFix(playerid, vehicleid)
{
	if(!IsPlayerConnected(playerid)) return 0;
	//if(!IsValidVehicle(vehicleid)) return 0;
	if(pData[playerid][pMechanicStatus] != 1) return 0;
	if(pData[playerid][pJob] == 11 || pData[playerid][pJob2] == 11)
	{
		if(GetNearestVehicleToPlayer(playerid, 3.8, false) == vehicleid)
		{
			if(pData[playerid][pActivityTime] >= 100 && IsValidVehicle(vehicleid))
			{
				...
				InfoTD_MSG(playerid, 8000, "Fix Tires done!");
				pData[playerid][pMechVeh] = vehicleid;
				KillTimer(pData[playerid][pMechanic]);
				TogglePlayerControllable(playerid, 1);
				pData[playerid][pMechanicStatus] = 0;
			...
			{
				Error(playerid, "Tire fix fail! Anda tidak berada didekat kendaraan tersebut!");
				KillTimer(pData[playerid][pMechanic]);
				TogglePlayerControllable(playerid, 1);
				pData[playerid][pMechanicStatus] = 0;
				HidePlayerProgressBar(playerid, pData[playerid][activitybar]);
