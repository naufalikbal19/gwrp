function MuatBarang(playerid)
{
	...
	InfoMsg(playerid, "Bawa muatan kembali ke ~y~Pelabuhan Megency ~w~ikutilah tanda.");
...
CMD:kargodentwot(playerid, params[])
{
    if(pData[playerid][pJob] == 8)
	{
		...
		ShowPlayerDialog(playerid, DIALOG_KARGO, DIALOG_STYLE_TABLIST_HEADERS, "{BABABA}Megency - {FFFFFF}Pilih Kargo", "Jenis\nBarang\nPertamina", "Pilih", "Tutup");
	}
	return 1;
}
