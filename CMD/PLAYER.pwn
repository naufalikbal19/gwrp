...
{
    ShowPlayerDialog(playerid, DIALOG_HBEMODE, DIALOG_STYLE_LIST, "Greenwich - HBE Mode", ""LG_E"Simple\n"LG_E"Modern", "Pilih", "Tutup");
    return 1;
}
CMD:belijerigen(playerid, params[])
{
    ...
    pData[playerid][pGas]++;
    InfoMsg(playerid, "Anda Sedang Membeli Jerigen mohon tunggu!");
	GivePlayerMoneyEx(playerid, -50);
	ShowProgressbar(playerid, "Sedang Mengisi..", 8);
...
{
	if(!IsPlayerInRangeOfPoint(playerid, 3.0, -1069.2753,-2322.0605,55.8431)) return ErrorMsg(playerid, "Anda sedang tidak berada di black market");
	ShowPlayerDialog(playerid, DIALOG_BSHOP, DIALOG_STYLE_TABLIST_HEADERS, "BlackShop", "Barang\tHarga\nPanel Hacking\t{7fff00}$200\nClip(1)\t{7fff00}$50\nClip(10)\t{7fff00}$500", "Beli", "Keluar");
	return 1;
}
...
CMD:credits(playerid)
{
	...
	strcat(line3, ""LB_E"Coder: "YELLOW_E"Rehan/Justin/Wisnu for Update\n");
	strcat(line3, ""LB_E"Developer: "YELLOW_E"Justin\n");
	strcat(line3, ""LB_E"Website: "YELLOW_E"https://discord.gg/GreenwichRp\n");
	format(line2, sizeof(line2), ""LB_E"Server Support: "YELLOW_E"%s & All SA-MP Team\n\n\
	"GREEN_E"Terima kasih telah bergabung dengan kami! Copyright © 2023 | Greenwich Roleplay.", pData[playerid][pName]);
	format(line1, sizeof(line1), "%s%s", line3, line2);
   	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E"Greenwich:RP: "WHITE_E"Server Credits", line1, "OK", "");
	return 1;
}
...
{
	if(!IsPlayerInRangeOfPoint(playerid, 3.0, 974.92, 1962.64, -88.67)) return ErrorMsg(playerid, "Anda harus berada di rumah sakit!");
...
{
	if(!IsPlayerInRangeOfPoint(playerid, 2.0, 114.67, 1165.16, 10019.16)) return ErrorMsg(playerid, "Anda harus berada di Kantor kepolisian!");
...
{
	if(!IsPlayerInRangeOfPoint(playerid, 2.0, 112.20, 1164.95, 10019.16)) return ErrorMsg(playerid, "Anda harus berada di kantor SAPD!");
...
{
	if(!IsPlayerInRangeOfPoint(playerid, 2.0, 114.67, 1165.16, 10019.16)) return ErrorMsg(playerid, "Anda harus berada di Kantor Polisi!");
...
{
	if(!IsPlayerInRangeOfPoint(playerid, 3.0, 1545.033935,-2177.635498,13.582808)) return ErrorMsg(playerid, "Kamu harus berada di Kantor Insurance!");
...
{
	if(!IsPlayerInRangeOfPoint(playerid, 3.0, 482.5168,-1305.0946,1021.6249)) return ErrorMsg(playerid, "Anda harus berada di Bank!");
...
{
	if(!IsPlayerInRangeOfPoint(playerid, 3.0, 482.5168,-1305.0946,1021.6249)) return ErrorMsg(playerid, "Anda harus berada di bank point!");
	new tstr[128];
...
CMD:mask(playerid, params[])
{
	...
	switch (pData[playerid][pMaskOn])
    {
        case 0:
        {
			...
			new nametag[128];
			format(nametag, sizeof(nametag), "[MASK : %d]", pData[playerid][pMaskID]);
			UpdateDynamic3DTextLabelText(Tagidplayer[playerid], 0xFFFFFFFF, nametag);
        ...
        case 1:
        {
			...
			new nametag[128];
			format(nametag, sizeof(nametag), "[ID : %d]", playerid);
			UpdateDynamic3DTextLabelText(Tagidplayer[playerid], 0xFFFFFFFF, nametag);
        }
    }
	return 1;
...
CMD:p(playerid, params[])
{
	...
	foreach(new ii : Player)
	{
		if(playerid == pData[ii][pCall])
		{
			...
			StreamTelpon[targetid] = SvCreateGStream(0xFFA200FF, "InPhone");
...
CMD:togphone(playerid, params[])
{
	...
	SuccesMsg(playerid, "Handphone berhasil dinyalakan!");
	}
	else
	{
	pData[playerid][pPhoneStatus] = 0;
	SuccesMsg(playerid, "Handphone berhasil dimatikan!");
	}
	return 1;
}
...
CMD:buy(playerid, params[])
{
	...
	if(IsPlayerInRangeOfPoint(playerid, 2.5, 921.7545,-1299.1313,14.0938))
	{
		...
		{
		ShowPlayerDialog(playerid, DIALOG_RESTOCK_PDG, DIALOG_STYLE_LIST, "Greenwich - Restock Pedagang", "Beras\nSambal\nTepung\nGula\nAyam Fillet", "Pilih", "Batal");
		}
		if(pData[playerid][pFaction] != 5)
		{
	...
	//Apotek
	if(IsPlayerInRangeOfPoint(playerid, 2.5, 964.37, 1955.88, -88.65))
	{
...
//rob atm sistem//
CMD:cmd57(playerid, params[])
{
	for(new i = 0; i < 6; i++)
	{
		TextDrawShowForPlayer(playerid, RobBankTD[i]);
	}
