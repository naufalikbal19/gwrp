...
public DCC_DM_EMBED(str[], pin, id[])
{
	...
	new DCC_Embed:embed = DCC_CreateEmbed(.title="Greenwich Roleplay", .image_url="https://media.discordapp.net/attachments/1107289439253835786/1122874355374895114/IMG-20230625-WA0044.jpg");
...
public CheckDiscordUCP(DiscordID[], Nama_UCP[])
{
	...
	{
		new ns[32];
		guild = DCC_FindGuildById("1123596265406615603");
		WARGA = DCC_FindRoleById("1113878973756227664");
...
{
	new id[21];
    if(channel != DCC_FindChannelById("1086881965464100869"))
		return 1;
    if(isnull(params))
	...
	{
		new str[256];
		format(str, sizeof(str), "```:x: Hai %s, anda sudah pernah mendaftar dan tidak bisa lagi mengambil Karcis.Silahkan ke channel <#1119615829311496223> untuk refund role dan di cek oleh staff!```", uname);
...
{
    new id[21];
    if(channel != DCC_FindChannelById("1086881965141143622"))
		return 1;
	new DCC_Embed:msgEmbed, msgField[256];
	format(msgField, sizeof(msgField), " Player Online Di Kota Greenwich: %s", number_format(Iter_Count(Player)));
	msgEmbed = DCC_CreateEmbed("", msgField, "", "", 0x00ff00, "Kota Greenwich | #MoreThanACommunity", "", "", "");
...
DCMD:ucp(user, channel, params[])
{
	...
	{
		new str[256];
		format(str, sizeof(str), "<a:emoji_35:1123596265406615603> Hai %s, Anda sudah pernah mendaftar dan tidak bisa lagi mengambil karcis", uname);
		DCC_SendChannelMessage(channel, str);
	}
	else
	{
		...
		mysql_format(g_SQL, characterQuery, sizeof(characterQuery), "SELECT * FROM `playerucp` WHERE `DiscordID` = '%s'", id);
		mysql_tquery(g_SQL, characterQuery, "CheckDiscordID", "ss", id, params);
		DCC_SendChannelMessage(channel, "<a:emoji_34:1106796247467163749> Ucp anda berhasil diverifikasi, silahkan cek message dari Greenwich Bot.");
	}
    cache_delete(ex);
	return 1;
}
...
{
    if(channel != DCC_FindChannelById("1086881963954159682"))
		return 1;

	new DCC_Channel:Info;
   	Info = DCC_FindChannelById("1088840277298532392");
...
DCMD:admins(user, channel, params[])
{
	...
	foreach(new i:Player)
	{
		...
		{
			format(line3, sizeof(line3), "%s\n%s(%s)\n", line3, pData[i][pName], pData[i][pUCP], GetStaffRank(i));
			count++;
		}
	}
	if(count > 0)
...
{
    new DCC_Embed:msgEmbed, msgField[256];
	format(msgField, sizeof(msgField), "172.105.113.56 IP ADDRESS");
	msgEmbed = DCC_CreateEmbed("", msgField, "", "", 0x00ff00, "Kota Greenwich | #GreenwichCommunity", "", "", "");
	DCC_SendChannelEmbedMessage(channel, msgEmbed);
	return 1;
}
...
DCMD:players(user, channel, params[])
{
	foreach(new i : Player)
	{
		new DCC_Embed:embed = DCC_CreateEmbed(.title = "Greenwich Roleplay");
...
{
    if(channel != DCC_FindChannelById("1086881964184842351"))
		return 1;
	...
	foreach(new ii : Player)
	{
		...
		{
			DCC_SendChannelMessage(channel, "`[GreenwichBOT]: Player tersebut online, Tidak Bisa Menggunakan ini.`");
	  		return 1;
	  	}
	}
	new query[512];
...
{
	if(channel != channelCS) return DCC_SendChannelMessage(channel, "`[GreenwichBOT]: Error you are not part of the admin team`");
	new target, cs[5], string[256];
	if(sscanf(params, "us[5]", target, cs)) return DCC_SendChannelMessage(channel, "`[GreenwichBOT]: Usage !setcs [PlayerName] [Yes/No]`");

	if(!strcmp(cs, "Yes", true))
	{
		if(!IsPlayerConnected(target)) return DCC_SendChannelMessage(channel, "```\n[Greenwich]: Error Player yang ingin anda atur sedang tidak ada di dalam kota, Silahkan gunakan \"/osetcs\"\n```");

		format(string, sizeof(string), "`[GreenwichBOT]: Error akun dengan nama %s cs nya telah di atur Ke [ACCEPT]`", pData[target][pCs]);
		...
		format(string, sizeof(string), "`[GreenwichBOT]: Akun dengan nama %s Character Storynya Berhasil Diatur | [ACCEPTED]`", pData[target][pName]);
	...
	else if(!strcmp(cs, "No", true))
	{
		...
		format(string, sizeof(string), "`[GreenwichBOT]: Error akun dengan nama %s cs nya telah di atur Ke [DENIED]`", pData[target][pCs]);
		if(!pData[target][pCs]) return DCC_SendChannelMessage(channel, string);

		pData[target][pCs] = 0;
		Servers(target, "Character Story Anda berhasil di atur | [DENIED]");

		format(string, sizeof(string), "`[GreenwichBOT]: Akun dengan nama %s Character Storynya Berhasil Diatur | [DENIED]`", pData[target][pName]);
		DCC_SendChannelMessage(channel, string);
	}
	return 1;
}
...
DCMD:osetcs(user, channel, params[])
{
	if(channel != channelCS) return DCC_SendChannelMessage(channel, "`[GreenwichBOT]: Error you are not part of the admin team`");
	new target, cs[5], string[256];
	if(sscanf(params, "ss[5]", target, cs)) return DCC_SendChannelMessage(channel, "`[GreenwichBOT]: Usage !osetcs [PlayerName] [Yes/No]`");

	if(!strcmp(cs, "Yes", true))
	{
		if(IsPlayerConnected(target)) return DCC_SendChannelMessage(channel, "```\n[Greenwich]: Error Player yang ingin anda atur sedang berada di dalam kota, silahkan gunakan /setcs\n```");
...
{
	if(channel != panelGreenwich) return DCC_SendChannelMessage(channel, "`[GreenwichBOT]: Error you are not part of the admin team`");
	...
	else DCC_SendChannelMessage(channel, "`[GreenwichBOT]: There are no sapd online`");
	return 1;
}
...
DCMD:samdonline(user, channel, params[])
{
	...
	else DCC_SendChannelMessage(channel, "`[GreenwichBOT]: There are no samd online`");
...
DCMD:checkucp(user, channel, params[])
{
	...
	new nameUcp[24];
	if(sscanf(params, "s[24]", nameUcp)) return DCC_SendChannelMessage(channel, "`[GreenwichBOT]: Usage !checkucp [UCP]`");
...
DCMD:jumlahucp(user, channel, params[])
{
	...
	new msg[256];
	format(msg, sizeof msg, "`[GreenwichBOT]: Registered ucp account is %d accounts`", registered_players);
...
DCMD:setucp(user, channel, params[])
{
	...
	channelSetUCP = DCC_FindChannelById("1119615829311496223");

	if(channel != channelSetUCP) return DCC_SendChannelMessage(channel, "`[GreenwichBOT]: Error you are not part of the admin team`");
	new getNameIC[256], getNameUCP[12];
	if(sscanf(params, "s[256]s[12]", getNameIC, getNameUCP)) return DCC_SendChannelMessage(channel, "`[GreenwichBOT]: Usage !setucpuser [Name IC] [Name UCP]`");
...
{
	if(channel != g_Discord_ReportAccept) return DCC_SendChannelMessage(channel, "`[GreenwichBOT]: Kamu tidak bisa menggunakan cmd ini!`");
