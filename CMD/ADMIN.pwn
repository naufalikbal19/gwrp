...
CMD:acmds(playerid)
{
	...
	{
		strcat(line3, "{ff0000}[Administrator]: {ffffff}/makequiz, /adminjail, /togooc /goto, /freeze, /unfreeze, /slap, /bt, /ft , /caps, /aitems, /astats, /adelays\n");
...
CMD:cmd55(playerid, params[])
{
    ...
    if(isnull(params))
        return SyntaxMsg(playerid, "[text]");

	new flyingtext[128];
	format(flyingtext, sizeof(flyingtext), "%s", params);
    ShowNotifyChat(playerid, flyingtext);
    return 1;
}
CMD:cmd56(playerid, params[])
{
	if(IsPlayerInRangeOfPoint(playerid, 50, 2184.32, -1023.32, 1018.68))
		return Error(playerid, "Anda tidak dapat melakukan ini jika sedang berada di OOC Zone");
	...
	new flyingtext[128];
	format(flyingtext, sizeof(flyingtext), "%s", params);
    ShowDarkChat(playerid, flyingtext);
    return 1;
}
...
CMD:giveredmoney(playerid, params[])
{
	...
	Servers(otherid, "Admin %s telah memberikan redmoney kepada anda dengan jumlah %s!", pData[playerid][pUCP], FormatMoney(money));
...
CMD:gmx(playerid, params[])
{
        ...
        if(pData[playerid][pAdmin] >= 6)
	    {
	    	if(CurGMX == 0)
	    	{
	    		// format(String, sizeof(String), "GMX:"RED_E" %s "WHITE_E"akan menutup kota untuk sementara dalam "YELLOW_E"30 detik", pData[playerid][pUCP]);
				// SendClientMessageToAll(COLOR_LBLUE, String);
				SendClientMessageToAll(COLOR_LBLUE, "Restart Server akan dilakukan.");
				SendClientMessageToAll(COLOR_LBLUE, "Jangan Lupa gunakan cmd /savestats");
				// SendClientMessageToAll(COLOR_LBLUE, "Untuk seluruh pemain harap untuk menggunakan command /savestats untuk berjaga jaga data rollback");
...
CMD:admins(playerid, params[])
{
	...
	strcat(location,"Rank\tAdmin Name\tPlayer ID\n",sizeof(location));
    foreach (new i : Player) 
	if (PlayerData[i][pAdmin] > 0 && !PlayerData[i][pAhide])
	{
		strcat(location,"Rank\tAdmin Name\tPlayer ID\n",sizeof(location));
	
	if(count)
	{
		format(lstr,sizeof(lstr), "%s\t%s\t%d\n", GetStaffRank(i), PlayerData[i][pUCP], i);
	}
		else format(lstr,sizeof(lstr), "%s\t%s\t%d\n", GetStaffRank(i), PlayerData[i][pUCP], i);
...
CMD:adminduty(playerid, params[])
{
	...
	new count = 0;
	new location[4080], lstr[596];
	strcat(location,"Rank\tNickname\tAdmin Name\tDuty Time(minutes)\n",sizeof(location));
    foreach (new i : Player) 
	if (PlayerData[i][pAdmin] > 0 && !PlayerData[i][pAhide])
	{
		strcat(location,"Rank\tNickname\tAdmin Name\tDuty Time(minutes)\n",sizeof(location));
	...
	if(count)
	{
		format(lstr,sizeof(lstr), "%s\t%s\t%s\t%d\n", GetStaffRank(i), pData[i][pName], PlayerData[i][pUCP], PlayerData[i][pAdminDutyTimeMinutes]);
	}
		else format(lstr,sizeof(lstr), "%s\t%s\t%s\t%d\n", GetStaffRank(i), pData[i][pName], PlayerData[i][pUCP], PlayerData[i][pAdminDutyTimeMinutes]);
		strcat(location,lstr,sizeof(location));
	}
	ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_TABLIST_HEADERS,"Staff Duty Time Greenwich",location,"Tutup","");
	return 1;
}
CMD:adminpoint(playerid, params[])
{
	if(pData[playerid][pAdmin] < 1)
		if(pData[playerid][pHelper] == 0)
		return ErrorMsg(playerid, "Anda tidak bisa akses perintah ini!");
	...
	new count = 0;
	new location[4080], lstr[596];
	strcat(location,"Rank\tNickname\tAdmin Name\tPoint\n",sizeof(location));
    foreach (new i : Player) 
	if (PlayerData[i][pAdmin] > 0 && !PlayerData[i][pAhide])
	{
		strcat(location,"Rank\tNickname\tAdmin Name\tPoint\n",sizeof(location));
	...
	if(count)
	{
		format(lstr,sizeof(lstr), "%s\t%s\t%s\t%d\n", GetStaffRank(i), pData[i][pName], PlayerData[i][pUCP], PlayerData[i][pAdminPoint]);
	}
		else format(lstr,sizeof(lstr), "%s\t%s\t%s\t%d\n", GetStaffRank(i), pData[i][pName], PlayerData[i][pUCP], PlayerData[i][pAdminPoint]);
		strcat(location,lstr,sizeof(location));
	}
...
CMD:aduty(playerid, params[])
{
	if(pData[playerid][pAdmin] < 1)
		if(pData[playerid][pHelper] == 0)
			return ErrorMsg(playerid, "Anda bukan staff Greenwich");
			
	// if(!strcmp(pData[playerid][pUCP], "None"))
	...
	if(!pData[playerid][pAdminDuty])
    {
		...
		if(pData[playerid][pAdmin] > 0)
		{
			...
			SendStaffMessage(COLOR_YELLOW, "{7fffd4}%s telah on duty sebagai staff.", pData[playerid][pUCP]);
			...
			AdminDutyTimer = SetTimerEx("ADminDutyHour", 60000, true, "i", playerid);
		}
		else
		{
			...
			pData[playerid][pAdminDuty] = 1;
			SetPlayerName(playerid, pData[playerid][pUCP]);
			SendStaffMessage(COLOR_YELLOW, "{7fffd4}%s telah on duty sebagai helper", pData[playerid][pUCP]);
			pData[playerid][AdminTag] = CreateDynamic3DTextLabel("Loading nametag...", 0xFFFFFFFF, 0.0, 0.0, 0.1, NT_DISTANCE, .attachedplayer = playerid, .testlos = 1);
			SetTimer("UpdateNametagAdmin", 1000, false);
		}
    }
    else
    {
       ...
       DestroyDynamic3DTextLabel(pData[playerid][AdminTag]);
        ...
        SendStaffMessage(COLOR_RED, "{7fffd4}%s telah off admin duty.", pData[playerid][pUCP]);
		new strings[500];
...
CMD:asay(playerid, params[]) 
{
    ...
    SendClientMessageToAllEx(COLOR_RED,"{ff0000}[STAFF] (%s{ff0000}) "YELLOW_E"%s: "LG_E"%s", GetStaffRank(playerid), pData[playerid][pUCP], ColouredText(text));
    return 1;
}
...
CMD:a(playerid, params[])
{
	...
	SendAdminMessage(COLOR_GREEN, "{3BBD44}(( Admin Chat | %s %s: %s ))", GetStaffRank(playerid), PlayerData[playerid][pUCP], params);
	return 1;
}
...
CMD:togooc(playerid, params[])
{
    ...
    if(TogOOC == 0)
    {
        SendClientMessageToAllEx(COLOR_RED, "GreenwichBot: {ffff00}Admin %s telah menonaktifkan global OOC chat.", pData[playerid][pUCP]);
        TogOOC = 1;
    }
    else
    {
        SendClientMessageToAllEx(COLOR_RED, "GreenwichBot: {ffff00}Admin %s telah mengaktifkan global OOC chat (DON'T SPAM).", pData[playerid][pUCP]);
        TogOOC = 0;
    }
    return 1;
}
...
CMD:o(playerid, params[])
{
    ...
    if(strlen(params) < 90)
    {
        ...
        {
            if(pData[playerid][pAdmin] == 1) SendClientMessageEx(i, COLOR_PINK2, "[G] ADMINISTRATOR | %s: {FFFFFF}%s", pData[playerid][pUCP], params);
            else if(pData[playerid][pAdmin] == 2) SendClientMessageEx(i, COLOR_PINK2, "[G] ADMINISTRATOR | %s: {FFFFFF}%s", pData[playerid][pUCP], params);
            else if(pData[playerid][pAdmin] == 3) SendClientMessageEx(i, COLOR_PINK2, "[G] ADMINISTRATOR | %s: {FFFFFF}%s", pData[playerid][pUCP], params);
            else if(pData[playerid][pAdmin] == 4) SendClientMessageEx(i, COLOR_PINK2, "[G] ADMINISTRATOR | %s: {FFFFFF}%s", pData[playerid][pUCP], params);
            else if(pData[playerid][pAdmin] == 5) SendClientMessageEx(i, COLOR_PINK2, "[G] ADMINISTRATOR | %s: {FFFFFF}%s", pData[playerid][pUCP], params);
            else if(pData[playerid][pAdmin] == 6) SendClientMessageEx(i, COLOR_PINK2, "[G] ADMINISTRATOR | %s: {FFFFFF}%s", pData[playerid][pUCP], params);
			else if(pData[playerid][pHelper] > 0 && pData[playerid][pAdmin] == 0)
			{
				SendClientMessageEx(i, COLOR_PINK2, "[G] ADMINISTRATOR | %s: {FFFFFF}%s", pData[playerid][pUCP], params);
			}
            else
            {
...
CMD:sendto(playerid, params[])
{
	...
	if(!strcmp(type,"ls")) 
	{
		...
		Servers(otherid, "Admin %s telah mengirim anda ke teleport spawn", pData[playerid][pUCP]);
...
CMD:freeze(playerid, params[])
{
	...
	Servers(otherid, "You have been frozen movements by admin %s.", pData[playerid][pUCP]);
    return 1;
}
...
CMD:unfreeze(playerid, params[])
{
	...
	Servers(otherid, "You have been unfrozen movements by admin %s.", pData[playerid][pUCP]);
    return 1;
}
...
CMD:arevive(playerid, params[])
{
    ...
    SendStaffMessage(COLOR_RED, "Staff %s have revived player %s.", pData[playerid][pUCP], ReturnName(otherid));
    Info(otherid, "Staff %s has revived your character.", pData[playerid][pUCP]);
    return 1;
}
...
CMD:spec(playerid, params[])
{
    ...
    SendStaffMessage(COLOR_WHITE, "{7fffd4}%s now spectating %s (ID: %d).", pData[playerid][pUCP], pData[otherid][pName], otherid);
...
CMD:slap(playerid, params[])
{
	...
	SendStaffMessage(COLOR_RED, "Admin %s telah men-slap player %s", pData[playerid][pUCP], pData[otherid][pName]);
...
CMD:ft(playerid, params[])
{
	...
	GetPlayerPos(otherid, POS[0], POS[1], POS[2]);
	SetPlayerPos(otherid, POS[0] + 5.0, POS[1], POS[2]);
	if(IsPlayerInAnyVehicle(otherid)) 
	{
		RemovePlayerFromVehicle(otherid);
		//OnPlayerExitVehicle(otherid, GetPlayerVehicleID(otherid));
...
CMD:bt(playerid, params[])
{
	...
	GetPlayerPos(otherid, POS[0], POS[1], POS[2]);
	SetPlayerPos(otherid, POS[0], POS[1] + 5.0, POS[2]);
	if(IsPlayerInAnyVehicle(otherid)) 
	{
		RemovePlayerFromVehicle(otherid);
		//OnPlayerExitVehicle(otherid, GetPlayerVehicleID(otherid));
...
CMD:caps(playerid, params[])
{
	...
	if(!GetPVarType(otherid, "Caps"))
	{
		...
		SendClientMessageToAllEx(COLOR_RED, "Server: {ffff00}Admin %s telah menon-aktifkan anti caps kepada player %s", pData[playerid][pUCP], pData[playerid][pName]);
	}
	else
	{
	    // Enable Caps
		DeletePVar(otherid, "Caps");
		SendClientMessageToAllEx(COLOR_RED, "Server: {ffff00}Admin %s telah meng-aktifkan anti caps kepada player %s", pData[playerid][pUCP], pData[playerid][pName]);
	}
	return 1;
}
...
function OWarnPlayer(adminid, NameToWarn[], warnReason[])
{
	...
	else
	{
		...
		SendClientMessageToAllEx(COLOR_RED, "Server: {ffff00}Admin %s telah memberi warning(offline) player %s. [Reason: %s]", pData[adminid][pUCP], NameToWarn, warnReason);
		new str[150];
...
function OJailPlayer(adminid, NameToJail[], jailReason[], jailTime)
{
	...
	else
	{
		...
		SendClientMessageToAllEx(COLOR_RED, "Server: {ffff00}Admin %s telah menjail(offline) player %s selama %d menit. [Reason: %s]", pData[adminid][pUCP], NameToJail, jailTime, jailReason);
		new str[150];
...
CMD:jail(playerid, params[])
{
	...
	if(reason[0] == '*')
	{
		SendClientMessageToAllEx(COLOR_PINK2, "ADMINISTRATOR | %s {ffffff}telah mempenjara pemain %s selama %d bulan.", pData[playerid][pUCP], pData[otherid][pName], timeSec);
		new str[150];
	...
	{
		SendClientMessageToAllEx(COLOR_PINK2, "ADMINISTRATOR | %s telah mempenjara pemain %s selama %d bulan Alasan -> %s", pData[playerid][pUCP], pData[otherid][pName], timeSec, reason);
		new str[150];
...
CMD:unjail(playerid, params[])
{
	...
	SendClientMessageToAllEx(COLOR_PINK2, "ADMINISTRATOR | %s {ffffff}telah membebaskan pemain %s dari penjara", pData[playerid][pUCP], pData[otherid][pName]);
	new str[150];
...
CMD:ban(playerid, params[])
{
	...
	new zstr[325];
	format(zstr, sizeof(zstr),"{FFFFFF}UCP Anda diblokir dari Greenwich Roleplay\n\nBerdasarkan Informasi:\nNama UCP: {ffffff}%s\nIP Address: {FFFFFF}%s\nAdmin On Duty: {ffffff}%s\nTanggal Diblokir: {FFFFFF}%i/%02d/%02d %02d:%02d\nReason: {FFFFFF}%s\nDurasi Blokir: {ffffff}%s\n\n"YELLOW_E"INGAT: {FFFFFF}Jika anda ingin bermain lagi dan bertobat, masuk ke discord: "YELLOW_E"bit.ly/Greenwich {ffffff}kemudian pilih chanel #request � unbanned � Greenwich", pData[playerid][pUCP], PlayerIP, PlayerData[playerid][pUCP], date[2], date[0], date[1], date[3], date[4], tmp);
	ShowPlayerDialog(otherid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, "{FFFFFF}Greenwich - UCP Blocked", zstr, "Tutup", "");

	new query[248];
	mysql_format(g_SQL, query, sizeof(query), "INSERT INTO banneds(name, ip, admin, reason, ban_date, ban_expire) VALUES ('%s', '%s', '%s', '%s', %i, %d)", giveplayer, PlayerIP, pData[playerid][pUCP], tmp, gettime(), ban_time);
	mysql_tquery(g_SQL, query);
	KickEx(otherid);
	return true;
}
...
CMD:banucp(playerid, params[])
{
	...
	if(datez != 0)
	{
		SendClientMessageToAllEx(COLOR_RED, "Server: {ffff00}Admin %s telah membanned player %s selama %d hari. {ffff00}[Reason: %s]", pData[playerid][pUCP], pData[otherid][pUCP], datez, tmp);
		new str[150];
	...
	{
		SendClientMessageToAllEx(COLOR_RED, "Server: {ffff00}Admin %s telah membanned permanent player %s. {ffff00}[Reason: %s]", pData[playerid][pUCP], pData[otherid][pUCP], tmp);
		new str[150];
	...
	new query[248];
	mysql_format(g_SQL, query, sizeof(query), "INSERT INTO banneds(name, ip, admin, reason, ban_date, ban_expire) VALUES ('%s', '%s', '%s', '%s', %i, %d)", pData[otherid][pUCP], PlayerIP, pData[playerid][pUCP], tmp, gettime(), ban_time);
	mysql_tquery(g_SQL, query);
	KickEx(otherid);
	return true;
}
...
CMD:obanucp(playerid, params[])
{
	...
	new query[248];
	mysql_format(g_SQL, query, sizeof(query), "INSERT INTO banneds(name, ip, admin, reason, ban_date, ban_expire) VALUES ('%s', '%s', '%s', '%s', %i, %d)", player, pData[playerid][pIP], pData[playerid][pUCP], reason, gettime(), datez);
	mysql_tquery(g_SQL, query);

	if(datez == 0)
	{
	SendClientMessageToAllEx(COLOR_RED, "Server: {ffff00}Admin %s has UCP Termporary banned %s \n reason : %s.", pData[playerid][pUCP], player,reason);
	new str[150];
	format(str,sizeof(str),"Admin: %s banned UCP %s!", pData[playerid][pUCP], player);
	LogServer("Admin", str);
	new dc[128];
	format(dc, sizeof(dc),  "```\nBanned Log : \n Admin %s has UCP temporary banned %s.\nReason : %s!.```", pData[playerid][pUCP], player, reason);
	SendDiscordMessage(3, dc);
	}
	else
	{
	SendClientMessageToAllEx(COLOR_RED, "Server: {ffff00}Admin %s has UCP banned %s for %d Days.\nReason : %s", pData[playerid][pUCP], player, datez,reason);
	new str[150];
	format(str,sizeof(str),"Admin: %s banned UCP %s!", pData[playerid][pUCP], player);
	LogServer("Admin", str);
	new dc[128];
	format(dc, sizeof(dc),  "```\nBanned Log : \n Admin %s has UCP banned %s for %d Days.\nReason : %s!.```", pData[playerid][pUCP], player, datez, reason);
	SendDiscordMessage(3, dc);
	}
...
CMD:osetcs(playerid, params[])
{
	...
	new player[24], cs;
	if(sscanf(params, "s[24]D(0)", player, cs))
	{
	    SyntaxMsg(playerid, "/osestcs <player name> option (1 for accept)");
	    Info(playerid, "Will set a character story player");
	    return true;
	}
	foreach(new ii : Player)
	{
	    ...
	    if(strfind(PlayerName, player, true) != -1)
		{
			Error(playerid, "Player is online, you can use /setcs on him.");
	  		return true;
	  	}
	}
	new query[248];
	mysql_format(g_SQL, query, sizeof(query), "UPDATE players SET characterstory='%d' WHERE username='%s'",cs, player);
	mysql_tquery(g_SQL, query);
	return 1;
}
...
function OnUnbanQueryData(adminid, BannedName[])
{
	if(cache_num_rows() > 0)
	{
		...
		SendClientMessageToAllEx(COLOR_RED, "Server: {ffff00}%s(%i) has unbanned %s from the server.", pData[adminid][pUCP], adminid, BannedName);
		new str[150];
...
function UnbanUCP(adminid, BannedName[])
{
	if(cache_num_rows() > 0)
	{
		...
		SendStaffMessage(COLOR_RED, "Server: {ffff00}%s(%i) has unbanned %s from the server.", pData[adminid][pUCP], adminid, BannedName);
		new str[150];
...
CMD:warn(playerid, params[])
{
	...
	pData[otherid][pWarn]++;
	SendClientMessageToAllEx(COLOR_RED, "Server: {ffff00}Admin %s telah memberikan warning kepada player %s. [Reason: %s] [Total Warning: %d/20]", pData[playerid][pUCP], pData[otherid][pName], reason, pData[otherid][pWarn]);
    return 1;
}
...
CMD:unwarn(playerid, params[])
{
	...
	Servers(otherid, "%s has unwarned 1 point your warnings.", pData[playerid][pUCP]);
    SendStaffMessage(COLOR_RED, "Admin %s has unwarned 1 point to %s's warnings.", pData[playerid][pUCP], pData[otherid][pName]);
    new str[150];
...
CMD:respawnsapd(playerid, params[])
{
	...
	SendStaffMessage(COLOR_RED, "%s has respawned SAPD vehicles.", pData[playerid][pUCP]);
	return 1;
}
...
CMD:respawnsags(playerid, params[])
{
	...
	SendStaffMessage(COLOR_RED, "%s has respawned SAGS vehicles.", pData[playerid][pUCP]);
	return 1;
}
...
CMD:respawnsamd(playerid, params[])
{
	...
	SendStaffMessage(COLOR_RED, "%s has respawned SAMD vehicles.", pData[playerid][pUCP]);
	return 1;
}
...
CMD:respawnsana(playerid, params[])
{
	...
	SendStaffMessage(COLOR_RED, "Admin %s has respawned SANA vehicles.", pData[playerid][pUCP]);
	return 1;
}
        ...
        if(GetVehicleModel(i))
        {
            ...
            if (((tempposx < radi) && (tempposx > -radi)) && ((tempposy < radi) && (tempposy > -radi)) && ((tempposz < radi) && (tempposz > -radi)))
            {
				if(IsVehicleEmpty(i))
				{
					...
					SendClientMessageToAllEx(COLOR_RED, "Server: {ffff00}Admin %s telah merespawn kendaraan disekitar dengan radius %d.", pData[playerid][pUCP], radi);
...
CMD:sethp(playerid, params[])
{
	...
	SendStaffMessage(COLOR_RED, "%s telah men set jumlah hp player %s", pData[playerid][pUCP], pData[otherid][pName]);
	Servers(otherid, "Admin %s telah men set hp anda", pData[playerid][pUCP]);
	new str[150];
...
CMD:setbone(playerid, params[])
{
	...
	SendStaffMessage(COLOR_RED, "%s telah men set jumlah Kondisi tulang player %s", pData[playerid][pUCP], pData[otherid][pName]);
	Servers(otherid, "Admin %s telah men set Kondisi tulang anda", pData[playerid][pUCP]);
	return 1;
}
...
CMD:setam(playerid, params[])
{
	...
	SendStaffMessage(COLOR_RED, "%s telah men set jumlah armor player %s", pData[playerid][pUCP], pData[otherid][pName]);
	Servers(otherid, "Admin %s telah men set armor anda", pData[playerid][pUCP]);
	new str[150];
...
CMD:akill(playerid, params[])
{
	...
	if(reason[0] != '*')
	{
		SendClientMessageToAllEx(COLOR_RED, "Servers: {ffff00}Admin %s has killed %s. "GREY_E"[Reason: %s]", pData[playerid][pUCP], pData[otherid][pName], reason);
	}
	else
	{
		SendClientMessageToAllEx(COLOR_RED, "Servers: {ffff00}Admin %s has killed %s.", pData[playerid][pUCP], pData[otherid][pName]);
	}
	return 1;
}
...
CMD:cd(playerid)
{
	...
	SendClientMessageToAllEx(COLOR_RED, "[SERVER] "LB_E"Admin %s has started a global countdown!", pData[playerid][pUCP]);
	return 1;
}
...
CMD:banip(playerid, params[])
{
	...
	SendClientMessageToAllEx(COLOR_RED, "Server: {ffff00}Admin %s(%d) IP banned address %s.", pData[playerid][pUCP], playerid, params);
	new str[150];
...
CMD:reloadweap(playerid, params[])
{
    ...
    Servers(otherid, "Admin %s have reload your weapons.", pData[playerid][pUCP]);
    return 1;
}
...
CMD:resetweap(playerid, params[])
{
    ...
    Servers(otherid, "Admin %s have reset your weapons.", pData[playerid][pUCP]);
    return 1;
}
...
CMD:setlevel(playerid, params[])
{
	...
	SendStaffMessage(COLOR_RED, "%s telah men set jumlah level player %s", pData[playerid][pUCP], pData[otherid][pName]);
	Servers(otherid, "Admin %s telah men set level anda", pData[playerid][pUCP]);
	new str[150];
...
CMD:sethbe(playerid, params[])
{
	...
	SendStaffMessage(COLOR_RED, "%s telah men set jumlah hbe player %s", pData[playerid][pUCP], pData[otherid][pName]);
	Servers(otherid, "Admin %s telah men set HBE anda", pData[playerid][pUCP]);
	return 1;
}
...
function SetName(otherplayer, playerid, nname[])
{
	if(!cache_num_rows())
	{
		...
		Servers(otherplayer, "Admin %s telah mengganti nickname anda menjadi (%s)", pData[playerid][pUCP], nname);
		Info(otherplayer, "Pastikan anda mengingat dan mengganti nickname anda pada saat login kembali!");
		SendStaffMessage(COLOR_RED, "Admin %s telah mengganti nickname player %s(%d) menjadi %s", pData[playerid][pUCP], oldname, otherplayer, nname);
		new str[150];
...
CMD:setbooster(playerid, params[])
{
	...
	if(dayz == 0)
	{
		...
		SendClientMessageToAllEx(COLOR_RED, "Server: {ffff00}Admin %s(%d) telah menset Roleplay Booster kepada %s(%d) ke level permanent time!", pData[playerid][pUCP], playerid, pData[otherid][pName], otherid);
	}
	else
	{
		pData[otherid][pBoostTime] = gettime() + (dayz * 86400);
		SendClientMessageToAllEx(COLOR_RED, "Server: {ffff00}Admin %s(%d) telah menset Roleplay Booster kepada %s(%d) selama %d hari!", pData[playerid][pUCP], playerid, pData[otherid][pName], otherid, dayz);
	}
...
CMD:setvip(playerid, params[])
{
	...
	if(dayz == 0)
	{
		...
		SendClientMessageToAllEx(COLOR_RED, "Server: {ffff00}Admin %s(%d) telah menset VIP kepada %s(%d) ke level %s permanent time!", pData[playerid][pUCP], playerid, pData[otherid][pName], otherid, GetVipRank(otherid));
		new str[150];
	...
	else
	{
		...
		SendClientMessageToAllEx(COLOR_RED, "Server: {ffff00}Admin %s(%d) telah menset VIP kepada %s(%d) selama %d hari ke level %s!", pData[playerid][pUCP], playerid, pData[otherid][pName], otherid, dayz, GetVipRank(otherid));
		new str[150];
...
CMD:setfaction(playerid, params[])
{
    ...
    if(fid < 0 || fid > 8)
        return Error(playerid, "You have specified an invalid faction ID 0 - 8.");
...
CMD:takemoney(playerid, params[])
{
	...
	SendClientMessageToAllEx(COLOR_RED, "Server: {ffff00}Admin %s(%i) has taken away money "RED_E"%s {ffff00}from %s", pData[playerid][pUCP], FormatMoney(money), pData[otherid][pName]);
	new str[150];
...
CMD:takegold(playerid, params[])
{
	...
	SendClientMessageToAllEx(COLOR_RED, "Server: {ffff00}Admin %s(%i) has taken away gold "RED_E"%d {ffff00}from %s", pData[playerid][pUCP], gold, pData[otherid][pName]);
	new str[150];
...
CMD:destroyveh(playerid, params[])
{
	...
	SendStaffMessage(COLOR_RED, "%s menghancurkan semua kendaraan yang dispawn admin.", pData[playerid][pUCP]);
	return 1;
}
...
CMD:agl(playerid, params[])
{
	...
	SendStaffMessage(COLOR_RED, "%s telah memberi sim kepada player %s", pData[playerid][pUCP], pData[otherid][pName]);
	Servers(otherid, "Admin %s telah memberi sim anda", pData[playerid][pUCP]);
	return 1;
}
//-----------------------------[ Admin Level 5 ]------------------
CMD:sethelperlevel(playerid, params[])
{
	...
	SendStaffMessage(COLOR_RED, "Admin %s telah menset %s(%d) sebagai staff helper level %s(%d)",  pData[playerid][pUCP], pData[otherid][pName], otherid, GetStaffRank(playerid), alevel);
	new str[150];
...
CMD:setmoney(playerid, params[])
{
	...
	Servers(otherid, "Admin %s telah mengset uang anda menjadi %s!",pData[playerid][pUCP], FormatMoney(money));
	new str[150];
...
CMD:givemoney(playerid, params[])
{
	...
	new str[150];
	format(str,sizeof(str),"~p~Admin %s ~w~telah memberikan uang kepada anda dengan jumlah ~y~%s!", pData[playerid][pUCP], FormatMoney(money));
	InfoMsg(otherid, str);
...
CMD:givemoneyall(playerid, params[])
{
	...
	foreach(new pid : Player)
	{
		...
		new str[500];
		format(str, sizeof(str), "Admin %s telah memberikan %s kepada semua player online", pData[playerid][pUCP], FormatMoney(money));
		InfoMsg(pid, str);
...
CMD:sethpall(playerid, params[])
{
	...
	new lstr[1024];
	format(lstr, sizeof(lstr), "ADMINISTRATOR | %s: {ffffff}Telah menyembuhkan semua player yang ada di kota", pData[playerid][pUCP]);
...
CMD:setstressall(playerid, params[])
{
    ...
    new lstr[1024];
	format(lstr, sizeof(lstr), "ADMINISTRATOR | %s: {ffffff}Telah mereset stress semua player yang ada di kota", pData[playerid][pUCP]);
	SendClientMessageToAll(COLOR_PINK2, lstr);
	foreach(new pid : Player)
	{
...
CMD:setbankmoney(playerid, params[])
{
	...
	Servers(otherid, "Admin %s telah mengset uang rekening bank anda menjadi %s!",pData[playerid][pUCP], FormatMoney(money));
	new str[150];
...
CMD:givebankmoney(playerid, params[])
{
	...
	Servers(otherid, "Admin %s telah memberikan uang rekening bank kepada anda dengan jumlah %s!", pData[playerid][pUCP], FormatMoney(money));
	new str[150];
...
CMD:setvw(playerid, params[])
{
	...
	Servers(otherid, "Admin %s telah men set Virtual World anda", pData[playerid][pUCP]);
	return 1;
}
...
CMD:setint(playerid, params[])
{
	...
	Servers(otherid, "Admin %s telah men set Interior anda", pData[playerid][pUCP]);
	return 1;
}
...
CMD:setgold(playerid, params[])
{
	...
	Servers(otherid, "Admin %s telah menset gold kepada anda dengan jumlah %d!", pData[playerid][pUCP], gold);
	new str[150];
...
CMD:givegold(playerid, params[])
{
	...
	Servers(otherid, "Admin %s telah memberikan gold kepada anda dengan jumlah %d!", pData[playerid][pUCP], gold);
	new str[150];
...
CMD:agive(playerid, params[])
{
	...
	if(IsPlayerConnected(playerid)) 
	{
		...
		if(strcmp(name,"bandage",true) == 0) 
		{
			...
			Info(otherid, "Admin %s telah berhasil memberikan perban kepada anda sejumlah %d.", pData[playerid][pUCP], ammount);
		...
		else if(strcmp(name,"medicine",true) == 0) 
		{
			...
			Info(otherid, "Admin %s telah berhasil memberikan medicine kepada anda sejumlah %d.", pData[playerid][pUCP], ammount);
			ApplyAnimation(playerid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
			ApplyAnimation(otherid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
		}
		else if(strcmp(name,"snack",true) == 0) 
		{
			...
			Info(otherid, "Admin %s telah berhasil memberikan snack kepada anda sejumlah %d.", pData[playerid][pUCP], ammount);
			ApplyAnimation(playerid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
			ApplyAnimation(otherid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
		}
		else if(strcmp(name,"sprunk",true) == 0) 
		{
			...
			Info(otherid, "Admin %s telah berhasil memberikan Sprunk kepada anda sejumlah %d.", pData[playerid][pUCP], ammount);
			ApplyAnimation(playerid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
			ApplyAnimation(otherid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
		}
		else if(strcmp(name,"material",true) == 0) 
		{
			...
			Info(otherid, "Admin %s telah berhasil memberikan Material kepada anda sejumlah %d.", pData[playerid][pUCP], ammount);
			ApplyAnimation(playerid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
			ApplyAnimation(otherid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
		}
		else if(strcmp(name,"component",true) == 0) 
		{
			...
			Info(otherid, "Admin %s telah berhasil memberikan Component kepada anda sejumlah %d.", pData[playerid][pUCP], ammount);
			ApplyAnimation(playerid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
			ApplyAnimation(otherid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
		}
		else if(strcmp(name,"marijuana",true) == 0) 
		{
			...
			Info(otherid, "Admin %s telah berhasil memberikan Marijuana kepada anda sejumlah %d.", pData[playerid][pUCP], ammount);
			ApplyAnimation(playerid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
			ApplyAnimation(otherid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
		}
		...
		{	
			...
			Info(otherid, "Admin %s telah berhasil memberikan Obat kepada anda sejumlah %d.", pData[playerid][pUCP], ammount);
			ApplyAnimation(playerid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
			ApplyAnimation(otherid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
		}
		else if(strcmp(name,"gps",true) == 0) 
		{
			...
			Info(otherid, "Admin %s telah berhasil memberikan GPS kepada anda sejumlah %d.", pData[playerid][pUCP], ammount);
			ApplyAnimation(playerid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
			ApplyAnimation(otherid, "DEALER", "shop_pay", 4.0, 0, 0, 0, 0, 0);
		}
	}
...
CMD:setprice(playerid, params[])
{
	...
	if(!strcmp(name, "materialprice", true))
    {
        ...
        MaterialPrice = price;
		Server_Save();
        SendAdminMessage(COLOR_RED, "%s set material price to %s.", pData[playerid][pUCP], FormatMoney(price));
    }
	else if(!strcmp(name, "lumberprice", true))
    {
        ...
        LumberPrice = price;
		Server_Save();
        SendAdminMessage(COLOR_RED, "%s set lumber price to %s.", pData[playerid][pUCP], FormatMoney(price));
    }
    else if(!strcmp(name, "ayamfilletprice", true))
    {
        ...
        AyamFillPrice = price;
		Server_Save();
        SendAdminMessage(COLOR_RED, "%s set ayamfillet price to %s.", pData[playerid][pUCP], FormatMoney(price));
    }
	else if(!strcmp(name, "componentprice", true))
    {
		...
		Server_Save();
        SendAdminMessage(COLOR_RED, "%s set component price to %s.", pData[playerid][pUCP], FormatMoney(price));
    }
	else if(!strcmp(name, "metalprice", true))
    {
        ...
        MetalPrice = price;
		Server_Save();
        SendAdminMessage(COLOR_RED, "%s set metal price to %s.", pData[playerid][pUCP], FormatMoney(price));
    }
	else if(!strcmp(name, "gasoilprice", true))
    {
        ...
        GasOilPrice = price;
		Server_Save();
        SendAdminMessage(COLOR_RED, "%s set gasoil price to %s.", pData[playerid][pUCP], FormatMoney(price));
    }
	else if(!strcmp(name, "coalprice", true))
    {
        ...
        CoalPrice = price;
		Server_Save();
        SendAdminMessage(COLOR_RED, "%s set coal price to %s.", pData[playerid][pUCP], FormatMoney(price));
    }
	else if(!strcmp(name, "productprice", true))
    {
        ...
        ProductPrice = price;
		Server_Save();
        SendAdminMessage(COLOR_RED, "%s set product price to %s.", pData[playerid][pUCP], FormatMoney(price));
    }
	else if(!strcmp(name, "medicineprice", true))
    {
        ...
        MedicinePrice = price;
		Server_Save();
        SendAdminMessage(COLOR_RED, "%s set medicine price to %s.", pData[playerid][pUCP], FormatMoney(price));
    }
	else if(!strcmp(name, "medkitprice", true))
    {
        ...
        MedkitPrice = price;
		Server_Save();
        SendAdminMessage(COLOR_RED, "%s set medkit price to %s.", pData[playerid][pUCP], FormatMoney(price));
    }
	else if(!strcmp(name, "foodprice", true))
    {
        ...
        FoodPrice = price;
		Server_Save();
        SendAdminMessage(COLOR_RED, "%s set food price to %s.", pData[playerid][pUCP], FormatMoney(price));
    }
	else if(!strcmp(name, "fishprice", true))
    {
        ...
        FishPrice = price;
		Server_Save();
        SendAdminMessage(COLOR_RED, "%s set fish price to %s.", pData[playerid][pUCP], FormatMoney(price));
    }
	else if(!strcmp(name, "gsprice", true))
    {
		...
		Server_Save();
        SendAdminMessage(COLOR_RED, "%s set gs price to %s.", pData[playerid][pUCP], FormatMoney(price));
    }
    else if(!strcmp(name, "obatprice", true))
    {
        ...
        ObatPrice = price;
		Server_Save();
        SendAdminMessage(COLOR_RED, "%s set obat price to %s.", pData[playerid][pUCP], FormatMoney(price));
    }
	return 1;
}
...
CMD:setstock(playerid, params[])
{
	...
	if(!strcmp(name, "material", true))
    {
        ...
        Material = stok;
		Server_Save();
        SendAdminMessage(COLOR_RED, "%s set material to %d.", pData[playerid][pUCP], stok);
    }
	else if(!strcmp(name, "component", true))
    {
        ...
        Component = stok;
		Server_Save();
        SendAdminMessage(COLOR_RED, "%s set component to %d.", pData[playerid][pUCP], stok);
    }
    else if(!strcmp(name, "ayamfillet", true))
    {
        ...
        AyamFill = stok;
		Server_Save();
        SendAdminMessage(COLOR_RED, "%s set ayamfillet to %d.", pData[playerid][pUCP], stok);
    }
	else if(!strcmp(name, "product", true))
    {
        ...
        Product = stok;
		Server_Save();
        SendAdminMessage(COLOR_RED, "%s set product to %d.", pData[playerid][pUCP], stok);
    }
	else if(!strcmp(name, "gasoil", true))
    {
        ...
        GasOil = stok;
		Server_Save();
        SendAdminMessage(COLOR_RED, "%s set gasoil to %d.", pData[playerid][pUCP], stok);
    }
	else if(!strcmp(name, "apotek", true))
    {
        ...
        Apotek = stok;
		Server_Save();
        SendAdminMessage(COLOR_RED, "%s set apotek stok to %d.", pData[playerid][pUCP], stok);
    }
    else if(!strcmp(name, "pedagang", true))
    {
        ...
        Pedagang = stok;
		Server_Save();
        SendAdminMessage(COLOR_RED, "%s set pedagang stok to %d.", pData[playerid][pUCP], stok);
    }
	else if(!strcmp(name, "food", true))
    {
        ...
        Food = stok;
		Server_Save();
        SendAdminMessage(COLOR_RED, "%s set food stok to %d.", pData[playerid][pUCP], stok);
    }
    else if(!strcmp(name, "obat", true))
    {
        ...
        ObatMyr = stok;
		Server_Save();
        SendAdminMessage(COLOR_RED, "%s set obat stok to %d.", pData[playerid][pUCP], stok);
    }
	new str[150];
...
CMD:telolet(playerid, params[])
{
	new vehicleid = GetPlayerVehicleID(playerid);
	if(GetVehicleModel(vehicleid) != 431)
		return ErrorMsg(playerid, "Anda tidak berada di dalam bus!");
	...
	foreach(new ii : Player)
	{
		if(IsPlayerInRangeOfPoint(ii, 35.0, x, y, z))
		{
			PlayAudioStreamForPlayer(ii, "https://cdn.discordapp.com/attachments/896667778948427776/1158710528298975232/sound_effect_-_telolet.mp3", x, y, z, 35.0, 1);
			Servers(ii, "/stopsong, /togsong");
		}
	}
	return 1;
...
CMD:setstat(playerid, params[])
{
	...
	if(IsPlayerConnected(playerid)) 
	{
        ...
        if(sscanf(params, "us[24]S()[32]", otherid, name, param))
		{
			...
			Info(playerid, "Names: Borax, paketborax, Kecubug, paketkecubung, uangmerah, toolkit");
			return 1;
		}
		...
		{			
			...
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Level {FFFF00}%s {ffffff}ke level %d.", pData[playerid][pUCP], ReturnName(otherid), ammount);
			AdminCMD(otherid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Level{ffffff} kamu ke %d.", pData[playerid][pUCP], ammount);
		...
		{			
			...
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Poin Level {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pUCP], ReturnName(otherid), ammount);
			AdminCMD(otherid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Poin Level{ffffff} kamu menjadi %d.", pData[playerid][pUCP], ammount);
		...
		{			
			...
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Gold {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pUCP], ReturnName(otherid), ammount);
			AdminCMD(otherid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Gold{ffffff} kamu menjadi %d.", pData[playerid][pUCP], ammount);
		...
		{			
			...
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Money {FFFF00}%s {ffffff}menjadi %s.", pData[playerid][pUCP], ReturnName(otherid), FormatMoney(ammount));
			AdminCMD(otherid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Money{ffffff} kamu menjadi %d.", pData[playerid][pUCP], ammount);
		...
		{			
			...
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Bank Money {FFFF00}%s {ffffff}menjadi %s.", pData[playerid][pUCP], ReturnName(otherid), FormatMoney(ammount));
			AdminCMD(otherid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Bank Money{ffffff} kamu menjadi %d.", pData[playerid][pUCP], ammount);
		...
		{			
			...
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Phone Credit {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pUCP], ReturnName(otherid), ammount);
			AdminCMD(otherid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Phone Credit{ffffff} kamu menjadi %d.", pData[playerid][pUCP], ammount);
		...
		{			
			...
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Health {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pUCP], ReturnName(otherid), ammount);
			AdminCMD(otherid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Health{ffffff} kamu menjadi %d.", pData[playerid][pUCP], ammount);
		...
		{			
			...
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Armour {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pUCP], ReturnName(otherid), ammount);
			AdminCMD(otherid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Armour{ffffff} kamu menjadi %d.", pData[playerid][pUCP], ammount);
		...
		{			
			...
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Hunger {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pUCP], ReturnName(otherid), ammount);
			AdminCMD(otherid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Hunger{ffffff} kamu menjadi %d.", pData[playerid][pUCP], ammount);
		...
		{			
			...
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Energy {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pUCP], ReturnName(otherid), ammount);
			AdminCMD(otherid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Energy{ffffff} kamu menjadi %d.", pData[playerid][pUCP], ammount);
		...
		{			
			...
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Job {FFFF00}%s {ffffff}menjadi %s.", pData[playerid][pUCP], ReturnName(otherid), GetJobName(ammount));
			AdminCMD(otherid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Job{ffffff} kamu menjadi %s.", pData[playerid][pUCP], GetJobName(ammount));
		...
		{			
			...
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Second Job {FFFF00}%s {ffffff}menjadi %s.", pData[playerid][pUCP], ReturnName(otherid), GetJobName(ammount));
			AdminCMD(otherid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Second Job{ffffff} kamu menjadi %s.", pData[playerid][pUCP], GetJobName(ammount));
		...
		{			
			...
			pData[otherid][pMedicine] = ammount;
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Medicine {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pUCP], ReturnName(otherid), ammount);
			AdminCMD(otherid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Medicine{ffffff} kamu menjadi %d.", pData[playerid][pUCP], ammount);
		...
		{			
			...
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Medkit {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pUCP], ReturnName(otherid), ammount);
			AdminCMD(otherid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Medicine{ffffff} kamu menjadi %d.", pData[playerid][pUCP], ammount);
		...
		{			
			...
			pData[otherid][pSnack] = ammount;
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Snack {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pUCP], ReturnName(otherid), ammount);
			AdminCMD(otherid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Snack{ffffff} kamu menjadi %d.", pData[playerid][pUCP], ammount);
		...
		{			
			...
			pData[otherid][pSprunk] = ammount;
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Sprunk {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pUCP], ReturnName(otherid), ammount);
			AdminCMD(otherid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Sprunk{ffffff} kamu menjadi %d.", pData[playerid][pUCP], ammount);
		...
		{			
			...
			pData[otherid][pBandage] = ammount;
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Bandage {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pUCP], ReturnName(otherid), ammount);
			AdminCMD(otherid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Bandage{ffffff} kamu menjadi %d.", pData[playerid][pUCP], ammount);
		...
		{			
			...
			pData[otherid][pMaterial] = ammount;
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Material {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pUCP], ReturnName(otherid), ammount);
			AdminCMD(otherid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Material{ffffff} kamu menjadi %d.", pData[playerid][pUCP], ammount);
		...
		{			
			...
			pData[otherid][pComponent] = ammount;
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Component {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pUCP], ReturnName(otherid), ammount);
			AdminCMD(otherid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Component{ffffff} kamu menjadi %d.", pData[playerid][pUCP], ammount);
		...
		{			
			...
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Food {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pUCP], ReturnName(otherid), ammount);
			AdminCMD(otherid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Food{ffffff} kamu menjadi %d.", pData[playerid][pUCP], ammount);
		...
		{			
			...
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Seed {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pUCP], ReturnName(otherid), ammount);
			AdminCMD(otherid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Seed{ffffff} kamu menjadi %d.", pData[playerid][pUCP], ammount);
		...
		{			
			...
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Potato {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pUCP], ReturnName(otherid), ammount);
			AdminCMD(otherid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Potato{ffffff} kamu menjadi %d.", pData[playerid][pUCP], ammount);
		...
		{			
			...
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Orange {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pUCP], ReturnName(otherid), ammount);
			AdminCMD(otherid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Orange{ffffff} kamu menjadi %d.", pData[playerid][pUCP], ammount);
		...
		{			
			...
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Wheat {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pUCP], ReturnName(otherid), ammount);
			AdminCMD(otherid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Wheat{ffffff} kamu menjadi %d.", pData[playerid][pUCP], ammount);
		...
		{			
			...
			pData[otherid][pMarijuana] = ammount;
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Marijuana {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pUCP], ReturnName(otherid), ammount);
			AdminCMD(otherid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Marijuana{ffffff} kamu menjadi %d.", pData[playerid][pUCP], ammount);
		...
		{			
			...
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Fish {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pUCP], ReturnName(otherid), ammount);
			AdminCMD(otherid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Fish{ffffff} kamu menjadi %d.", pData[playerid][pUCP], ammount);
		...
		{			
			...
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Worm {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pUCP], ReturnName(otherid), ammount);
			AdminCMD(otherid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Worm{ffffff} kamu menjadi %d.", pData[playerid][pUCP], ammount);
		...
		{			
			...
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Family {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pUCP], ReturnName(otherid), ammount);
			AdminCMD(otherid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Family{ffffff} kamu menjadi %d.", pData[playerid][pUCP], ammount);
		...
		{			
			...
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Lockpick {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pUCP], ReturnName(otherid), ammount);
			AdminCMD(otherid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Lockpick{ffffff} kamu menjadi %d.", pData[playerid][pUCP], ammount);
		...
		{			
			...
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Borax {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pUCP], ReturnName(otherid), ammount);
			AdminCMD(otherid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Borax{ffffff} kamu menjadi %d.", pData[playerid][pUCP], ammount);
		...
		{			
			...
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Kecubung {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pUCP], ReturnName(otherid), ammount);
			AdminCMD(otherid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Kecubung{ffffff} kamu menjadi %d.", pData[playerid][pUCP], ammount);
		...
		{			
			...
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Paket Kecubung {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pUCP], ReturnName(otherid), ammount);
			AdminCMD(otherid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Paket Kecubung{ffffff} kamu menjadi %d.", pData[playerid][pUCP], ammount);
		...
		{			
			...
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Paket Borax {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pUCP], ReturnName(otherid), ammount);
			AdminCMD(otherid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Paket Borax{ffffff} kamu menjadi %d.", pData[playerid][pUCP], ammount);
		...
		{			
			...
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Uang Merah {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pUCP], ReturnName(otherid), ammount);
			AdminCMD(otherid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Uang Merah{ffffff} kamu menjadi %d.", pData[playerid][pUCP], ammount);
		...
		{			
			if(sscanf(param, "i", ammount))
			{
				return SyntaxMsg(playerid, "/setstat [playerid] [toolkit] [ammount]");
			}

			pData[otherid][pToolkit] = ammount;
			AdminCMD(playerid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Toolkit {FFFF00}%s {ffffff}menjadi %d.", pData[playerid][pUCP], ReturnName(otherid), ammount);
			AdminCMD(otherid, "{FFFF00}%s {ffffff}telah menyetel {15D4ED}Toolkit {ffffff} kamu menjadi %d.", pData[playerid][pUCP], ammount);

			mysql_format(g_SQL, query, sizeof(query), "UPDATE players SET toolkit = %i WHERE reg_id = %i", pData[otherid][pToolkit], pData[otherid][pID]);
	    	mysql_tquery(g_SQL, query);
		}
		else 
		{
...
CMD:clearallchat(playerid)
{
	...
	SendAdminMessage(COLOR_RED, "%s {FFFFFF}telah membersihkan chat box.", pData[playerid][pUCP]);
	return 1;
}
...
CMD:aclaimpv(playerid, params[])
{
	...
	foreach(new i : PVehicles)
	{
		if(pvData[i][cClaim] > 0 && pvData[i][cClaimTime] > 0)
		{
			if(pvData[i][cOwner] == pData[otherid][pID])
			{
				...
				Info(otherid, "Kamu telah mengclaim kendaraan dengan model %s di bantu oleh admin %s", GetVehicleModelName(pvData[i][cModel]), pData[playerid][pUCP]);
			}
	...
	else
	{
		...
		Info(otherid, "Kamu berhasil mengclaim %d kendaraan anda di bantu oleh admin %s!", found, pData[playerid][pUCP]);
	}
	return 1;
}
...
forward AdminDutyHour(playerid);
public AdminDutyHour(playerid)
{
	if(pData[playerid][pAdminDuty] < 1)
		return KillTimer(AdminDutyTimer);

	pData[playerid][pAdminDutyHour] += 1;
	if(pData[playerid][pAdminDutyHour] == 360)
	{
		Servers(playerid, "Anda telah Duty Admin selama 1 Jam");
	}
	return 1;
}
