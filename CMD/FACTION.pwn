...
CMD:menu(playerid, params[])
{
    ...
    {
        ShowPlayerDialog(playerid, DIALOG_MENU, DIALOG_STYLE_LIST, "Greenwich - Pedagang Menu","Ayam Goreng\nSteak Ayam\nStarling\nCappucino\nMilxMax\nWater","Pilih","Batal");
	}
	return 1;
}
...
CMD:r(playerid, params[])
{
    ...
    if(pData[playerid][pFaction] == 1) {
        ...
        SendFactionMessage(6, COLOR_RADIO, "** [HoliJek] %s(%d) %s: %s", GetFactionRank(playerid),  pData[playerid][pFactionRank], pData[playerid][pName], text);
		format(mstr, sizeof(mstr), "[<RADIO>]\n* %s *", text);
		SetPlayerChatBubble(playerid, mstr, COLOR_RADIO, 10.0, 3000);
    }
	else if(pData[playerid][pFaction] == 5) {
        SendFactionMessage(5, COLOR_RADIO, "** [Pedagang] %s: %s", pData[playerid][pName], text);
		format(mstr, sizeof(mstr), "[<RADIO>]\n* %s *", text);
		SetPlayerChatBubble(playerid, mstr, COLOR_RADIO, 10.0, 3000);
    }
	else if(pData[playerid][pFaction] == 7) {
        SendFactionMessage(7, COLOR_RADIO, "** [Mechanic] %s: %s", pData[playerid][pName], text);
		format(mstr, sizeof(mstr), "[<RADIO>]\n* %s *", text);
		SetPlayerChatBubble(playerid, mstr, COLOR_RADIO, 10.0, 3000);
    }
    else
...
CMD:gov(playerid, params[])
{
	...
	{
		new lstr[1024];
		format(lstr, sizeof(lstr), "Mechanic | %s: {ffffff}%s", pData[playerid][pName], params);
		SendClientMessageToAll(COLOR_GREEN, lstr);
	}
	return 1;
}
...
CMD:arrest(playerid, params[])
{
	...
	if(!IsPlayerInRangeOfPoint(playerid, 3.0, 105.52, 1209.61, 10016.12))
		return Error(playerid, "You must be near an arrest point.");
...
CMD:mix(playerid, params[])
{
    ...
    if(IsPlayerInRangeOfPoint(playerid, 2.0, 966.28, 1955.89, -88.65))
    {
...
CMD:revive(playerid, params[])
{
    ...
    Info(otherid, "%s has revived you.", pData[playerid][pUCP]);
    return 1;
}
