...
House_Type(houseid)
{
	if(hData[houseid][hType] == 1)
	{
	    switch(random(3))
		{
			case 0:
			{
				hData[houseid][hIntposX] = -15.490423;
				hData[houseid][hIntposY] = -468.735626;
				hData[houseid][hIntposZ] = -31.894290;
				hData[houseid][hIntposA] = 182.145034;
...
{
	return Error(playerid, "Fitur ditutup sementara");
}
// CMD:sellhouse(playerid, params[])
// {
...
// 	}
// 	ShowPlayerDialog(playerid, DIALOG_SELL_HOUSES, DIALOG_STYLE_LIST, "Nopix - Jual Rumah", CMDSString, "Jual", "Batal");
// 	return 1;
// }

CMD:myhouse(playerid)
{
	if(GetOwnedHouses(playerid) == -1) return ErrorMsg(playerid, "Kamu tidak memiliki rumah.");
	//if(!Player_OwnsBusiness(playerid, id)) return Error(playerid, "You don't own this business.");
	new hid, _tmpstring[128], count = GetOwnedHouses(playerid), CMDSString[1024];
	CMDSString = "";
	new lock[128];
	...
	ShowPlayerDialog(playerid, DIALOG_MY_HOUSES, DIALOG_STYLE_LIST, "Megency - House", CMDSString, "Pilih", "Batal");
	return 1;
}
