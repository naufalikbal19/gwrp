...
Bisnis_BuyMenu(playerid, bizid)
{
    ...
    switch(bData[bizid][bType])
    {
        ...
        {
            new MarketHarga[560];
	    	format(MarketHarga,3,"HARGA : %s", FormatMoney(bData[bizid][bP][0]));//Water
	    	TextDrawSetString(NoMarket[25], MarketHarga);
	    	format(MarketHarga,25,"HARGA : %s", FormatMoney(bData[bizid][bP][1]));//Milx Max
	    	TextDrawSetString(NoMarket[28], MarketHarga);
	    	//format(MarketHarga,1000,"HARGA : %s", FormatMoney(bData[bizid][bP][2]));//Kebab
	    //	TextDrawSetString(MarketIndoSans[21], MarketHarga);
	    	format(MarketHarga,5,"HARGA : %s", FormatMoney(bData[bizid][bP][3]));//Roti
	    	TextDrawSetString(NoMarket[30], MarketHarga);
	    	format(MarketHarga,7,"HARGA : %s", FormatMoney(bData[bizid][bP][4]));//Snack
	    	TextDrawSetString(NoMarket[23], MarketHarga);
	    	format(MarketHarga,1,"HARGA : %s", FormatMoney(bData[bizid][bP][5]));//Starling
	    	TextDrawSetString(NoMarket[21], MarketHarga);
	    	format(MarketHarga,1000,"HARGA : %s", FormatMoney(bData[bizid][bP][6]));//Cappucino
        ...
        case 3:
        {
			...
			TextDrawShowForPlayer(playerid, BajuTD[19]);
			TextDrawShowForPlayer(playerid, BajuTD[20]);
...
Bisnis_Refresh(id)
{
    if(id != -1)
    {
		...
		{
			format(tstr, 128, "Owner: {FFFF00}%s\n"WHITE_E"Nama Bisnis: %s\nTekan "LG_E"'ALT' {FFFFFF}Untuk Membeli", bData[id][bOwner], bData[id][bName]);
			bData[id][bLabelPoint] = CreateDynamic3DTextLabel(tstr, COLOR_YELLOW, bData[id][bExtposX], bData[id][bExtposY], bData[id][bExtposZ], 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, 0, bData[id][bInt]);
   			if(IsValidDynamic3DTextLabel(bData[id][bLabel]))
            DestroyDynamic3DTextLabel(bData[id][bLabel]);
        }
        else
        {
            format(string, sizeof(string), "Bisnis ini dijual\nHarga: {FFFF00}%s\n{FFFFFF}Type: {FFFF00}%s\n"WHITE_E"Tekan "LG_E"N {FFFFFF}untuk membeli bisnis\n"LG_E"ALT {FFFFFF}Untuk Membeli", FormatMoney(bData[id][bPrice]), type);
		}
        ...
        bData[id][bLabel] = CreateDynamic3DTextLabel(string, COLOR_WHITE, bData[id][bExtposX], bData[id][bExtposY], bData[id][bExtposZ], 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, 0, bData[id][bInt]);
		
		if(bData[id][bType] == 1)
		{
			bData[id][bMap] = CreateDynamicMapIcon(bData[id][bPointX], bData[id][bPointX], bData[id][bPointX], 50, -1, -1, -1, -1, 700.0);
		}
		else if(bData[id][bType] == 2)
		{
			bData[id][bMap] = CreateDynamicMapIcon(bData[id][bPointX], bData[id][bPointX], bData[id][bPointX], 17, -1, -1, -1, -1, 700.0);
		}
		else if(bData[id][bType] == 3)
		{
			bData[id][bMap] = CreateDynamicMapIcon(bData[id][bPointX], bData[id][bPointX], bData[id][bPointX], 45, -1, -1, -1, -1, 700.0);
		}
		else if(bData[id][bType] == 4)
		{
			bData[id][bMap] = CreateDynamicMapIcon(bData[id][bPointX], bData[id][bPointX], bData[id][bPointX], 25, -1, -1, -1, -1, 700.0);
		}
		else if(bData[id][bType] == 5)
		{
			bData[id][bMap] = CreateDynamicMapIcon(bData[id][bPointX], bData[id][bPointX], bData[id][bPointX], 35, -1, -1, -1, -1, 700.0);
		}
		else
		{
			DestroyDynamicMapIcon(bData[id][bMap]);
...
CMD:mybis(playerid)
{
	...
	ShowPlayerDialog(playerid, DIALOG_MY_BISNIS, DIALOG_STYLE_LIST, "Megency - Bisnis Saya", CMDSString, "Pilih", "Batal");
	return 1;
}
...
CMD:bm(playerid, params[])
{
    ...
    ShowPlayerDialog(playerid, BISNIS_MENU, DIALOG_STYLE_LIST, "Megency - Bisnis Menu","Bisnis Info\nGanti Nama\nPendapatan Bisnis\nHarga Product\nRequest Restock","Pilih","Tutup");
    return 1;
}
