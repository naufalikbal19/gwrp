...
{	
	...
	if(up_seconds == 60)
	{
	    ...
	    if(up_minutes == 60)
	    {
			...
			DCC_AddEmbedField(msgEmbed, "greenwich time now", tstr);
			DCC_SetEmbedColor(msgEmbed, 0x00ff00);
			DCC_SetEmbedFooter(msgEmbed, "greenwich Server Status || #staytune");
...
ptask playerTimer[1000](playerid)
{
	if(pData[playerid][IsLoggedIn] == true)
	{
		...
		{
			pData[playerid][pAdminDutyTime]++;
			if(pData[playerid][pAdminDutyTime] >= 60)
			{	
				pData[playerid][pAdminDutyTimeMinutes] ++;
				pData[playerid][pAdminDutyTime] = 0;
			}
			if(pData[playerid][pAdminDutyTimeMinutes] >= 60)
			{	
