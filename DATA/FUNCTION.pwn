...
function OnPlayerDataLoaded(playerid, race_check)
{
	...
	if(cache_num_rows() > 0)
	{
		...
		{
			new str[400];
			format(str, sizeof(str), "UCP: {15D4ED}%s\n{ffffff}Silahkan masukkan PIN yang sudah di kirimkan oleh GreenwichBot", pData[playerid][pUCP]);
			ShowPlayerDialog(playerid, DIALOG_VERIFYCODE, DIALOG_STYLE_INPUT, "{BABABA}Greenwich - {ffffff}Verify Account", str, "Input", "Batal");
		}
...
function UpdateNametagAdmin()
{
    for(new i = 0, j = GetPlayerPoolSize(); i <= j; i++)
    {
        if(IsPlayerConnected(i))
        {
            ...
            if(armour > 1.0)
            {
                format(nametag, sizeof(nametag), "{BABABA}On Duty\n%s [%d]\n"YELLOW_E"%s", pData[i][pUCP], i, GetStaffRank(i));
            }
            else
            {
                format(nametag, sizeof(nametag), "{BABABA}On Duty\n%s [%d]\n"YELLOW_E"%s", pData[i][pUCP], i, GetStaffRank(i));
...
function YANGTIM(playerid)
{
	...
	{
		DCC_SetBotActivity("Greenwich V1.0");
		STATUS_BOT2 = false;
	}
	return 1;
}
...
function AssignPlayerData(playerid)
{
	...
	cache_get_value_name_int(0, "admindutytime", pData[playerid][pAdminDutyTime]);
	cache_get_value_name_int(0, "admindutytimeminutes", pData[playerid][pAdminDutyTimeMinutes]);
	cache_get_value_name_int(0, "adminpoint", pData[playerid][pAdminPoint]);
	...
	cache_get_value_name_int(0, "panel", pData[playerid][pPanel]);
	...
	cache_get_value_name_int(0, "clip", pData[playerid][pClip]);
	cache_get_value_name_int(0, "toolkit", pData[playerid][pToolkit]);
...
function OnPlayerRegister(playerid)
{
	...
	pData[playerid][pPosX] = 1918;
	pData[playerid][pPosY] = -2430;
	pData[playerid][pPosZ] = 23.5;
	pData[playerid][pPosA] = 93.90;
	...
	new clip = 0;
	mysql_format(g_SQL, query, sizeof(query), "SELECT clip FROM players WHERE clip='%d'", clip);
	mysql_tquery(g_SQL, query, "clip", "id", playerid, clip);
	
	new kayu = 0;
	mysql_format(g_SQL, query, sizeof(query), "SELECT kayu FROM players WHERE kayu='%d'", kayu);
	mysql_tquery(g_SQL, query, "clip", "id", playerid, kayu);

	SetSpawnInfo(playerid, NO_TEAM, 0, 1918, -2430, 23.5, 93.90, 0, 0, 0, 0, 0, 0);
	SpawnPlayer(playerid);
	return 1;
}
...
{
	SetPlayerPositionEx(playerid, 612.89, -2675.74, 2.20, 166.20, 2000);
...
function OnOBanQueryData(adminid, NameToBan[], banReason[], banTime)
{
	...
	else
	{
		...
		if(banTime != 0)
	    {
			...
			SendClientMessageToAllEx(COLOR_RED, "Server: "GREY2_E"Admin %s telah membanned offline player %s selama %d hari. [Reason: %s]", pData[adminid][pUCP], NameToBan, banTime, banReason);
		}
		else
		{
			Servers(adminid, "You have permanent-banned %s (IP: %s) from the server.", NameToBan, PlayerIP);
			SendClientMessageToAllEx(COLOR_RED, "Server: "GREY2_E"Admin %s telah membanned offline player %s secara permanent. [Reason: %s]", pData[adminid][pUCP], NameToBan, banReason);
		}
		new query[512];
		mysql_format(g_SQL, query, sizeof(query), "INSERT INTO banneds(name, ip, admin, reason, ban_date, ban_expire) VALUES ('%s', '%s', '%s', '%s', UNIX_TIMESTAMP(), %d)", NameToBan, PlayerIP, pData[adminid][pUCP], banReason, datez);
		mysql_tquery(g_SQL, query);
	}
	return true;
}
...
function CheckPlayerSpawn3Titik(playerid)
{
	...
	{
		SetPlayerPos(playerid, 1918, -2430, 23.5);
