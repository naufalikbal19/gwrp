...
function OnPlayerGiveInvItem(playerid, userid, itemid, name[], value)
{
	...
	{
		format(str, sizeof(str), "Removed_%dx", pData[playerid][pGiveAmount]);
		ShowItemBox(playerid, "toolkit", str, 19921, 2);

		format(str, sizeof(str), "Received_%dx", pData[playerid][pGiveAmount]);
		ShowItemBox(userid, "Botol", str, 19921, 2);

		ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
		ApplyAnimation(userid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);

		pData[playerid][pToolkit] -= value;
		pData[userid][pToolkit] += value;
	}
	...
	else if(!strcmp(name, "Cocaine", true))
	{
		...
		format(str, sizeof(str), "Received_%dx", pData[playerid][pGiveAmount]);
		ShowItemBox(userid, "Cocaine", str, 1580, 2);

		ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
		ApplyAnimation(userid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);

		pData[playerid][pCocaine] -= value;
		pData[userid][pCocaine] += value;
	}
	else if(!strcmp(name, "Panel_Hack", true))
	{
		format(str, sizeof(str), "Removed_%dx", pData[playerid][pGiveAmount]);
		ShowItemBox(playerid, "Panel_Hack", str, 363, 2);

		format(str, sizeof(str), "Received_%dx", pData[playerid][pGiveAmount]);
		ShowItemBox(userid, "Panel_Hack", str, 363, 2);

		ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
		ApplyAnimation(userid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);

		pData[playerid][pPanel] -= value;
		pData[userid][pPanel] += value;
	}
	...
	{
		format(str, sizeof(str), "REMOVED_%dx", pData[playerid][pGiveAmount]);
		ShowItemBox(playerid, string, str, InventoryData[playerid][itemid][invModel], 2);

		format(str, sizeof(str), "ADD_%dx", pData[playerid][pGiveAmount]);
		ShowItemBox(userid, string, str, InventoryData[playerid][itemid][invModel], 2);

		ApplyAnimation(playerid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);
		ApplyAnimation(userid, "DEALER", "shop_pay", 4.1, 0, 1, 1, 0, 0, 1);

		pData[playerid][pClip] -= pData[playerid][pGiveAmount];
		pData[userid][pClip] += pData[playerid][pGiveAmount];
	}
...
public OnPlayerUseItem(playerid, itemid, name[])
{
	...
	{
	    ShowProgressbar(playerid, "Menggunakan Cocaine..", 5);
	    ApplyAnimation(playerid,"SMOKING","M_smk_in",4.0, 1, 0, 0, 0, 0, 1);
	    
		pData[playerid][pCocaine] -= 1;
		pData[playerid][pHealth] += 5.0;

		SetPlayerHealth(playerid, pData[playerid][pHealth]);

		new string[128];
		format(string, sizeof(string), "%s Menggunakan Cocaine.", GetRPName(playerid));
		SetPlayerChatBubble(playerid, string, COLOR_PURPLE, 30.0, 10000);

 		ShowItemBox(playerid, "Cocaine", "Menggunakan_1x", InventoryData[playerid][itemid][invModel], 2);
		Inventory_Close(playerid);
	}
	else if(!strcmp(name, "Panel_Hack"))
	{
	    if(IsPlayerInRangeOfPoint(playerid, 1, 1690.892456,-2237.770751,13.539621))
		{
			callcmd::cmd57(playerid, "");
        }
	...
	else if(!strcmp(name, "toolkit"))
	{
		if(pData[playerid][pMechVeh] == INVALID_VEHICLE_ID)
		{
			...
			if(vehicleid == INVALID_VEHICLE_ID) return ErrorMsg(playerid, "Anda tidak berada di dekat kendaraan apapun.");
			if(pData[playerid][pActivityTime] > 5) return Error(playerid, "Anda sedang memperbaiki kendaraan!");
		...
		if(GetNearestVehicleToPlayer(playerid, 3.5, false) == pData[playerid][pMechVeh])
		{
			...
			format(Dstring, sizeof(Dstring), "%sTire Fix \t35 Component\n", Dstring);
			format(Dstring, sizeof(Dstring), "%sFill Gass oil \t50 Component\n", Dstring);
			format(Dstring, sizeof(Dstring), "%sSpray Car\t40 Component\n", Dstring);
			format(Dstring, sizeof(Dstring), "%sPaint Job Car\t60 Component\n", Dstring);
			if(IsPlayerInRangeOfPoint(playerid, 50.0, 2373.05, -1913.47, 13.57))
			{
				...
				format(Dstring, sizeof(Dstring), "%sNitro 1\t150 Component\n", Dstring);
				format(Dstring, sizeof(Dstring), "%sNitro 2\t200 Component\n", Dstring);
				format(Dstring, sizeof(Dstring), "%sNitro 3\t250 Component\n", Dstring);
				format(Dstring, sizeof(Dstring), "%sNeon\t300 Component\n", Dstring);
			}
	...
	else if(!strcmp(name, "Clip"))
	{
		if(GetPlayerAmmo(playerid) <= 1)
		{
			if(GetPlayerWeapon(playerid) == WEAPON_DEAGLE)
			{
				...
				// SetPlayerAmmo(playerid, WEAPON_DEAGLE, 7);
			...
			{
				ErrorMsg(playerid,"[Inventory] Kamu Tidak Memegang Senjata");
			}
		}
		else
		{
			ErrorMsg(playerid,"[Inventory] Senjata Tersebut Masih Memilik Clip");
		}
	}
...
stock BarangMasuk(playerid)
{
		...
		Inventory_Set(playerid, "toolkit", 19921, pData[playerid][pToolkit], pData[playerid][pToolkit]);
		...
		//panel hacking//
		Inventory_Set(playerid, "Panel_Hack", 363, pData[playerid][pPanel], pData[playerid][pPanel]);
		//clip system//
