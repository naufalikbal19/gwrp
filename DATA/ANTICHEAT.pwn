...
public OnPlayerTeleport(playerid, Float:distance)
{
	if((AntiCheatKontol) && pData[playerid][pAdmin] < 2)
	{
	    if(!IsPlayerInRangeOfPoint(playerid, 4.0, pData[playerid][pPosX], pData[playerid][pPosY], pData[playerid][pPosZ]))
	    {
			...
			{
		    	SendClientMessageToAllEx(COLOR_RED, "[MagniFicentBOT]: %s"WHITE_E" telah dikick otomatis oleh %s, Alasan: Teleport hacks", ReturnName(playerid), SERVER_BOT);
		    	new dc[128];
...
public OnPlayerAirbreak(playerid)
{
	if((AntiCheatKontol) && pData[playerid][pAdmin] < 2)
	{
		...
		else
		{
		    ...
		    SendClientMessageToAllEx(COLOR_RED, "[MagniFicentBOT]: %s"WHITE_E" telah dibanned otomatis oleh %s, Alasan: Airbreak", ReturnName(playerid), SERVER_BOT);
		    new dc[128];
...
NgecekCiter(playerid)
{
	if(gettime() > pData[playerid][pACTime])
	{
		...
		if((AntiCheatKontol) && GetPlayerState(playerid) == PLAYER_STATE_DRIVER && GetVehicleSpeed(GetPlayerVehicleID(playerid)) > 350 && pData[playerid][pAdmin] < 2)
		{
			...
			{
                new dc[128];
                SendClientMessageToAllEx(COLOR_RED, "[MagniFicentBOT]: %s"WHITE_E" telah dikick otomatis oleh %s, Alasan: Speed hacks", ReturnName(playerid), SERVER_BOT);
		...
		{
            new dc[128];
            SendClientMessageToAllEx(COLOR_RED, "[MagniFicentBOT]: %s"WHITE_E" telah dikick otomatis oleh %s, Alasan: Jetpack hacks", ReturnName(playerid), SERVER_BOT);
		...
		if((AntiCheatKontol) && GetPlayerState(playerid) == PLAYER_STATE_ONFOOT)
		{
			switch(GetPlayerAnimationIndex(playerid))
			{
			    case 958, 1538, 1539, 1543:
			    {
                    ...
                    {
                        new dc[128];
                        SendClientMessageToAllEx(COLOR_RED, "[MagniFicentBOT]: %s"WHITE_E" telah dikick otomatis oleh %s, Alasan: Flying hacks", ReturnName(playerid), SERVER_BOT);
			...
			GetPlayerArmour(playerid, armor);
  			...
  			if(!(gettime() - pData[playerid][pLastUpdate] > 5))
  			{
				if(floatround(armor) > floatround(pData[playerid][pArmour]) && gettime() > pData[playerid][pACTime] && gettime() > pData[playerid][pArmorTime] && pData[playerid][pAdmin] < 2)
				{
					...
					{
                        new dc[128];
                        SendClientMessageToAllEx(COLOR_RED, "[MagniFicentBOT]: %s"WHITE_E" telah dikick otomatis oleh %s, Alasan: Armor hacks", ReturnName(playerid), SERVER_BOT);
