...
CMD:ar(playerid, params[])
{
    ...
    SendStaffMessage(COLOR_RED, "%s has accepted %s(%d) report.", pData[playerid][pUCP], pData[ReportData[reportid][rPlayer]][pName], ReportData[reportid][rPlayer]);
    pData[playerid][pAdminPoint] ++;
    Servers(ReportData[reportid][rPlayer], "ACCEPT REPORT: {FF0000}%s {FFFFFF}accept your report.", pData[playerid][pUCP]);
    Report_Remove(reportid);
    return 1;
}
...
CMD:dr(playerid, params[])
{
    ...
    SendStaffMessage(COLOR_RED, "%s has denied %s(%d) report.", pData[playerid][pUCP], pData[ReportData[reportid][rPlayer]][pName], ReportData[reportid][rPlayer]);
    Servers(ReportData[reportid][rPlayer], "DENY REPORT: {FF0000}%s {FFFFFF}denied your report: %s.", pData[playerid][pUCP], msg);
...
CMD:reports(playerid, params[])
{
    ...
    for (new i = 0; i != MAX_REPORTS; i ++)
    {
        if(ReportData[i][rExists])
        {
            ...
            ShowPlayerDialog(playerid, DIALOG_REPORTS, DIALOG_STYLE_TABLIST_HEADERS,"Kota greenwich - Daftar Laporan",gstr,"Pilih","Tutup");
        }
        else
        {
...
CMD:clearreports(playerid, params[])
{
    ...
    SendStaffMessage(COLOR_RED, "%s has removed all reports on the server.", pData[playerid][pUCP]);
    return 1;
}
