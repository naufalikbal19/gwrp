...
{
	new dquery[2048];
	format(dquery, sizeof(dquery), "UPDATE familys SET name='%s', leader='%s', motd='%s', color='%d', extposx='%f', extposy='%f', extposz='%f', extposa='%f',intposx='%f', intposy='%f', intposz='%f', intposa='%f', fint='%d', fSkin1='%d', fSkin2='%d', fSkin3='%d', fSkin4='%d', fSkin5='%d'",
	fData[id][fName],
	fData[id][fLeader],
	fData[id][fMotd],
	fData[id][fColor],
...
CMD:fedit(playerid, params[])
{
    ...
    if(sscanf(params, "ds[24]S()[128]", fid, type, string))
    {
        ...
        SendClientMessage(playerid, COLOR_YELLOW, "[NAMES]:{FFFFFF} location, interior, name, skin, leader, safe, money, marijuana");
        return 1;
    }
